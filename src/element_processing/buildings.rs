...
fn generate_roof_only_structure(
    ...
    {
        GLASS
    } else if let Some(block) = element
        .tags
        .get("roof:colour")
...
fn generate_roof(
    ...
    // roof:colour tag: highest priority, overrides preset and random selection.
    let roof_colour_block: Option<Block> = element
        .tags
        .get("roof:colour")
        .and_then(|c| color_text_to_rgb_tuple(c))
        .map(get_building_wall_block_for_color);
    if let Some(block) = roof_colour_block {
        config.roof_block = block;
    }

    let roof_orientation = element.tags.get("roof:orientation").map(|s| s.as_str());

    // For flat roofs: prefer roof:colour > preset override > floor_block.
    let flat_roof_block = roof_colour_block
