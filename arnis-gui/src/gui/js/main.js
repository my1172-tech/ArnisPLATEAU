...
let selectedWorldFormat = 'luanti'; // Default to Java
...
function initSavePathSetting() {
  ...
  // CityGML file picker button
  const lod2Browse = document.getElementById('plateau-lod2-browse');
  const lod2Input = document.getElementById('plateau-lod2-path');
  if (lod2Browse && lod2Input) {
    lod2Browse.addEventListener('click', async () => {
      try {
        const picked = await invoke('gui_pick_citygml_file');
        if (picked) {
          lod2Input.value = picked;
        }
      } catch (error) {
        console.error("CityGML file picker failed:", error);
      }
    });
  }
}
...
async function startGeneration() {
  try {
    ...
    var gsi_enabled = document.getElementById("enable-gsi-toggle") ? document.getElementById("enable-gsi-toggle").checked : false;
    var plateau_enabled = document.getElementById("enable-plateau-toggle") ? document.getElementById("enable-plateau-toggle").checked : false;
    var plateau_lod2_path = document.getElementById("plateau-lod2-path") ? document.getElementById("plateau-lod2-path").value.trim() || null : null;
    ...
    await invoke("gui_start_generation", {
        ...
        plateauLod2Path: plateau_lod2_path
    });
